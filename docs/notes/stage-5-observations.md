...
- **Partial refund UI.** Current RefundPanel does full-amount only. Stripe's API supports `amount:` parameter; extending would need a number input and a server-side validation against `order.amount - already_refunded`. Deferred as out-of-prototype-scope. *Audit note:* the idempotency key `order:<uuid>:refund` is full-refund-only. If partial refunds land, the key MUST include the amount or a client-supplied attempt id (`order:<uuid>:refund:<amount>` or `:<client-uuid>`) — otherwise two partial refunds on the same order collide on Stripe's idempotency cache. Comment in `payments.ts` flags this.
- **Refund reason.** `stripe.refunds.create` is called with `reason: "requested_by_customer"` (Stage 5 audit addition) — the closest fit for a button-initiated refund. Main app should plumb a reason dropdown if "duplicate" or "fraudulent" reasons become operationally useful.
...
- **Application-level idempotence via `refunds.list`.** Stage 5 audit (codex Q6 MEDIUM) flagged a stale-local-state path: if the initial `charge.refunded` webhook is missed, the order row stays `succeeded`, Stripe's 24h idempotency-key cache expires, and a retry would create a duplicate refund. Fix applied: `POST /api/payments/refund` pre-checks `stripe.refunds.list({ payment_intent })` and reuses an existing refund if one exists. Covers both the <24h concurrent-double-submit case (via Stripe's idempotency key + StripeIdempotencyError catch) AND the >24h stale case (via the list pre-check). No schema change.
- **Shape-validation 500 UX gap.** Stage 5 audit (code-reviewer m3, codex Q7) flagged that if Stripe ships a novel `Refund.status`, the server returns 500 `{type: "refund_shape_unexpected", refundId}` after the refund is already created at Stripe. The `charge.refunded` webhook will still fire and flip the order row; the order row converges correctly. The gap is client UX: the button shows error even though the refund is in-flight. Prototype treats this as "fail loud" and relies on the log line (with `refundId` for reconciliation) as the alertable signal — main app should wire an alert on the `refund_shape_unexpected` log entry per the integration checklist.
